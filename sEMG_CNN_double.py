...
for sub_test in range(0,num_subject):
    ...
    X_test = X_test.reshape((int(num_rep[sub_test])*2, window_size/2, num_channel))    
    ...
    X_train = np.empty((0, window_size/2, num_channel))
    ...
    for sub_train in range(0, num_subject):
        ...
        if sub_train != sub_test:
            ...
            x_train = x_train.reshape((int(num_rep[sub_test])*2, window_size/2, num_channel))    
    ...
    with graph.as_default():
        ...
        flat = tf.reshape(max_pool_5, (-1, 250 * 128))
